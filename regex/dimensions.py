...
def masscyst_max_size(text):
    """
    Interprets results of regex function.
    :param text: The input text to be searched and passed on to regex function.
    :return: the float found to be the greatest dimension for the identified mass of interest, empty string if none found
    """
    results = find_phrases(text) 
    max_dim = ''
    if len(results) != 0:
        for result in results:
            if results[result][0] != 'no dimensions':
                matches = re.findall(r'\d+\.?\d*', str(results[result][0]))
                if matches:
                    max_dim = max(float(num) for num in matches)               
    return max_dim

# DECLARE INPUT FILES HERE:
# Folder Location:
path = r"R:\NYULH-Urology-Registry\PRJ_833_Kidney_Nephrectomy_Renal_Fn_EHR\PRJ_847_Kidney_GPT_Renalmass"
# File Name
# File should have features narrative_imp_combined, impression_fillednarrative, and segmented_narrative
df_gpt_renalmass_200 = pd.read_excel(fr'{path}\df_radreport_renalmass_toclassify_random_200_samples_04Apr2025.xlsx')
df_gpt_renalmass_400 = pd.read_excel(fr'{path}\df_radreport_renalmass_toclassify_random_400_samples_18Apr2025.xlsx')
df = pd.concat([df_gpt_renalmass_200, df_gpt_renalmass_400], ignore_index=True)
...
df['masscyst_max_size_re'] = df['narrative_imp_combined'].apply(masscyst_max_size)
...
df['masscyst_max_size_re_ds'] = df.apply(
    lambda row:  masscyst_max_size(row['segmented_narrative']) if row['solidmasscomplexcyst_re_fs'] == 1 else '',
    axis=1
)


# Output file
df.to_excel('output_dimensions{today_date}.xlsx', index=False)