...
def multiple_masscyst_measured(text):
    """
    Interprets results of regex function.
    :param text: The input text to be searched and passed on to regex function.
    :return: 0 if not multifocal, 1 if multifocal.
    """
    if len(find_phrases(text)) > 1:
        return 1
    return 0
...
df['multiple_masscyst_measured_re'] = df['narrative_imp_combined'].apply(multiple_masscyst_measured)
...
df['multiple_masscyst_measured_re_ds'] = df.apply(
    lambda row: multiple_masscyst_measured(row['segmented_narrative']) if row['solidmasscomplexcyst_re_fs'] == 1 else '',
    axis=1
)


# Output file
df.to_excel('output_multifocal{today_date}.xlsx', index=False)